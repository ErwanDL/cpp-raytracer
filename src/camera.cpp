...
PerspectiveCamera::PerspectiveCamera(const Point3& location, const Point3& target,
                                     const Vector3& upguide, float fov, float aspectRatio)
    : location(location), forward((target - location).normalized()), up(0.0f), right(0.0f),
      height(std::tan(fov)), width(0.0f) {
...
Ray PerspectiveCamera::makeRay(const Point2& point) const {
    Vector3 direction{forward + point.u * width * right + point.v * height * up};
